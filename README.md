...
4. **Item ID**\: идентификатор товара, который был куплен. Это связывает покупку с конкретным товаром в вашей системе. Поле всегда имеет значение `1` . Нужно для возможного расширения функционала.
...
3. ### **История покупок**
   ...
   * **`GET /users/{id}/purchases`** - Получение списка покупок пользователя. Успешный код: `200`.
     *Параметры запроса*\: Нет
     *Ответ*\: массив объектов покупок, каждый из которых включает **`id`**, **`user_id`**, **`transaction_id`**, **`item_id`**, **`quantity`**, **`created_at`**, **`updated_at`**
   * **`POST /users/{id}/purchases`** - Создание новой записи о покупке после завершения транзакции. Успешный код: `201`.
     *Параметры запроса*\: **`transaction_id`**, **`item_id`**, **`quantity`**
     *Ответ*\: **`purchase_id`**, **`user_id`**, **`transaction_id`**, **`item_id`**, **`quantity`**, **`created_at`**, **`updated_at`**
