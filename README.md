...
4. ~~**Item ID**\: идентификатор товара, который был куплен. Это связывает покупку с конкретным товаром в вашей системе. Поле всегда имеет значение `1` . Нужно для возможного расширения функционала. Данный атрибут передавать не нужно, он будет установлен автоматически.~~ **Атрибут будет скрыт в API**.
