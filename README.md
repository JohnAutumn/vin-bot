...
Сущность "платёж" объединяет транзакции, совершённые в контексте попытки пополнения баланса. Это нужно для случаев, когда есть несколько попыток оплаты. Например, если на счету пользователя было недостаточно средств - в системе появится соответствующая транзакция, и статус платежа будет "**failed**". Однако, если пользователь изменит карту оплаты и произойдёт успешная оплата - будет создана транзакция об успешной оплате и статус платежа будет "**completed**". Помимо timestamps и id у данной сущности только одно поле:

1. **Status** - Статус платежа, основанный на транзакциях. Имеет 3 возможных значения:
   * `pending` - дефолтный статус только созданного платежа
   * `failed` - платёж завершился с ошибкой
   * `completed` - платёж прошёл успешно

*Статусом транзакций управляет только платёжная система посредством вэбхуков.*

**Примечание**\: *платёж должен создаваться при нажатии на кнопку в боте "оплатить" (или подобную). В контексте платежа может быть множество транзакций, но транзакция со статусом **`completed` может быть только одна.***

### Автомобиль

1. **Vin** - vin код автомобиля
2. **Name** - марка и модель автомобиля
3. **Year** - год выпуска

*На текущий момент в сущности автомобиля есть поля "**url**" и "**status**", которые в дальнейшем будут удалены*
...
2. ### **Платежи**
   ...
   * **`GET /api/payments/{id}`**<span style="color:red;">**\:tg**</span><span style="color:red;"> </span>- Получение информации о платеже по его `id`. Успешный код: `200`.

     *Параметры запроса*\: нет

     *Ответ*\: `success`, `data`
   * **`POST /api/payments`**<span style="color:red;">**\:tg**</span><span style="color:red;"> </span>- Создание нового платежа. Возвращает данные созданного платежа. Успешный код: `201`.

     *Параметры запроса*\: нет

     *Ответ*\: `success`, `data`

     **Пример успешного создания/получения платежа**

     ```json
     {
         "success": true,
         "data": {
             "updated_at": "2023-08-18T13:12:25.000000Z",
             "created_at": "2023-08-18T13:12:25.000000Z",
             "id": 6,
             "status": "pending"
         }
     }
     ...
     **Примечание:** *при формировании платежа в системе **kassify**, в параметре `us_id` необходимо передать `id` платежа, созданного в API.*
